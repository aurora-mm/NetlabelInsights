# Mahorka version of Netlabel Insights
...
ui <- fluidPage(
  ...
  titlePanel("Netlabel Insights: Mahorka"),
  hr(),
  fluidRow(
    column(
      width = 3,
      ...
      hr(),
      h6("The Netlabel Insights dashboard for Mahorka provides an overview of download data from the Internet Archive across three historical periods. It allows users to explore the distribution of artist downloads and generate a hypothetical compilation using stratified sampling."),
      ...
      h6("The Compilation Maker generates a list of 10-14 artists for a hypothetical Mahorka compilation, based on historical data and stratified sampling. The compilation would feature artists whose albums have received fewer than 500 downloads, between 500 and 2000 downloads, and more than 2000 downloads. This ensures a mix of emerging and established artists from all time periods. Adjusting the Number of Artists slider generates a new compilation suggestion, drawn from Mahorka artist pool."),
    ),
    column(
      width = 9,
      tabsetPanel(
...
server <- function(input, output, session) {
  mahorka_data <- reactiveVal(NULL)
  cache_file <- "cache.rds"

  # Function for loading data
  load_mahorka_data <- function() {
    ...
    artist_lookup <- tribble(
      ~alias, ~canonical,
      "va", "various artists",
      "adrian juarez", "adrián juárez",
      "bedouindrone", "bedouin drone",
      "julien palomo + fabien robbe", "fabien robbe + julien palomo",
      "illi adato, buğra giritlioğlu, yannis saxonis", "illi adato, bugra giritlioglu, yannis saxonis"
    )
  ...
  observeEvent(input$load_data, {
    mahorka_data(load_mahorka_data())
  })
  ...
  output$plot_period1 <- renderPlot({
    req(mahorka_data())
    generate_period_plot(1, mahorka_data(), "goldenrod3")
  })
  output$plot_period2 <- renderPlot({
    req(mahorka_data())
    generate_period_plot(2, mahorka_data(), "darkseagreen4")
  })
  output$plot_period3 <- renderPlot({
    req(mahorka_data())
    generate_period_plot(3, mahorka_data(), "darkorchid4")
  })

  # Generate compilation
  output$compilation <- renderTable({
    req(mahorka_data())
    sampled_artists <- mahorka_data() %>%
